...
while [[ $# -gt 0 ]]; do
  ...
  case $key in
    ...
    -n)
      ...
      ;;
    -gen)
      echo "Flag -gen passed -> Generating new genesis."
      overwrite="gen-genesis"; shift
      ;;
    --no-install)
...
if [[ $overwrite == "n" || $overwrite == "N" ]]; then
  evmd start "$TRACE" \
    ...
    --p2p.persistent_peers="6348e7f70a4100c37c69b250e6081f3abcd3ae13@192.168.1.165:26656" \
    --evm.min-tip=0 \
    --home "$CHAINDIR" \
